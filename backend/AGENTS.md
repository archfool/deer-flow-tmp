...
│   │           ├── workflows/         # Durable task definitions/instances, DAG scheduler, repositories
│   │           ├── subject_memory/    # Versioned owner/subject-scoped business documents
...
- **Harness** (`packages/harness/deerflow/`): Publishable agent framework package (`deerflow-harness`). Import prefix: `deerflow.*`. Contains agent orchestration, tools, sandbox, models, MCP, skills, config, durable workflows, and versioned subject memory — everything domain-neutral needed to build and run agents.
- **App** (`app/`): Unpublished application code. Import prefix: `app.*`. Contains the FastAPI Gateway API, IM channel integrations (Feishu, Slack, Telegram, DingTalk), and domain verticals such as insurance.
...
### Durable Workflow Runtime (`packages/harness/deerflow/workflows/`)

The workflow runtime is the domain-neutral control plane for business tasks
that must survive conversation turns. It deliberately does not own the chat
loop: every message still enters the lead agent, which may operate on any
owner-scoped task instance.

- `TaskDefinition` is an immutable/versioned DAG of `WorkflowStep` records.
- `SkillDefinition` adds executable input/output/tool/side-effect metadata;
  `SKILL.md` remains the human/model instruction layer, not the state machine.
- `WorkflowEngine` runs ready siblings concurrently and stops at missing-input,
  confirmation, suspended, failed, or completed barriers.
- `TaskInstance` persists input snapshots, step outputs, pending questions,
  optional parent-task linkage, and an optimistic revision.
- `InMemoryTaskRepository` supports memory mode/tests;
  `SqlTaskRepository` persists `workflow_tasks` plus append-only
  `workflow_task_events` on SQLite/PostgreSQL.
- Owner filtering is mandatory for every read/write. Multiple active tasks can
  coexist for one thread/customer; conversational focus is an app/UI concern,
  not exclusive ownership of the lead agent.

Full contract and interruption semantics:
[`packages/harness/deerflow/workflows/README.md`](packages/harness/deerflow/workflows/README.md).

### Subject Memory (`packages/harness/deerflow/subject_memory/`)

Subject memory stores authoritative/versioned business documents separately
from the LLM-maintained user memory above.

- Composite scope: `(owner_id, subject_id, memory_type)`.
- Current snapshots live in `subject_memories`; committed JSON Pointer patches
  are appended to `subject_memory_revisions`.
- Model inference must enter `subject_memory_candidates`; accepting a candidate
  performs the same optimistic compare-and-swap as an explicit update.
- `None` versus an empty collection remains a domain-level distinction (for
  example, unknown policies are not equivalent to confirmed no policies).
- `SqlSubjectMemoryRepository` and `InMemorySubjectMemoryRepository` have the
  same application contract.

See
[`packages/harness/deerflow/subject_memory/README.md`](packages/harness/deerflow/subject_memory/README.md).

### Insurance Vertical (`app/insurance/`)

The first app-layer vertical uses the generic primitives without introducing a
harness-to-app dependency:
...
Detailed package map, safety notes, and replacement points:
[`app/insurance/README.md`](app/insurance/README.md).
...
DeerFlow's application tables (`runs`, `threads_meta`, `feedback`, `users`, `run_events`, the four `channel_*` tables, `workflow_*`, and `subject_memory*`) are owned by alembic via a **hybrid bootstrap** strategy. LangGraph's checkpointer tables (`checkpoints`, `checkpoint_blobs`, `checkpoint_writes`, `checkpoint_migrations`) live in the same database but are owned by LangGraph and excluded from alembic's view via `migrations/_env_filters.py::include_object`.
...
- `migrations/versions/0003_workflows_subject_memory.py` — adds durable task/event and versioned subject-memory/candidate tables
